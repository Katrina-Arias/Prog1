...
/* Otras funciones de la biblioteca string.hpara el manejo de cadenas. */
...
void main(void)
{
    ...
    i = strcmp(cad0, cad1);
    /* La función strcmppermite comparar dos cadenas de caracteres. Si la
    ...
    i = strlen(cad0);
    /* La función strlenobtiene la longitud —el número de caracteres— de una cadena. */ 
    printf("\nLongitud cadena cad0: %d", i);
    
    i = strlen(cad1);
    printf("\nLongitud cadena cad1: %d", i);
    
    c = strchr(cad1, 'G');    /* ces un apuntador de tipo caracter.*/
    /* La función strchrbusca la posición en la que se encuentra un
    ...
    if(c != NULL)
    {
           c3 = *c;       /* c3toma el contenido de la celda de memoria a la que apunta c.*/
